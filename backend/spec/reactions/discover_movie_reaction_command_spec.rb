    ...
    describe "#to_h" do
        ...
        context "when all options are given" do
            ...
            it "return valid hash" do
                ...
                expect(command.to_h).to eq({ reaction_id: reaction.id, user_id: user.id, language: "en.US", region: "FR", adult: "false", min_date: "2001", min_score: "5" })
            end
        end
        ...
        context "when options aren't given" do
            ...
            it "return valid hash" do
                command = DiscoverMovieReactionCommand.new(options)
                expect(command.to_h).to eq({ reaction_id: reaction.id, user_id: user.id, language: "fr-FR", region: "FR", adult: "false", min_date: "1975", min_score: "7" })
            end
        end
    end
end